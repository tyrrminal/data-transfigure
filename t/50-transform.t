...
like(dies { Data::Transform->new->add_transformers(undef)},
  qr/^Cannot register undef/,
  'attempt to register undef');

my $msg = q{Can't locate NonexistentClass.pm in @INC};
like(dies { Data::Transform->new->add_transformers('NonexistentClass') }, qr/^\Q$msg/, 
  'attempt to register non-existent class');

like(dies { Data::Transform->new->add_transformers('File::Spec') }, 
...
like(dies { Data::Transform->new->add_transformers('Data::Transform::Base') }, 
...
like(bless({ day => 3, month => 4, year => 2005 }, 'MyDateTime'), 
...
is($t->transform([[[{title => 'War and Peace'}, {date => $dt}]]]),
...
is($t->transform({ title => 'War and Peace', pages => 1200 }), 
  { title => '//War and Peace//', pages => '//1200//' }, 
  'apply overridden default transformer' );

$t = Data::Transform->new();
$t->add_transformer_at("/book/author" => Data::Transform::Type->new(
  type => 'MyApp::Person',
  handler => sub ($data) {
